from utils.pybullet_tools.pr2_primitives import iterate_approach_path
from utils.pybullet_tools.utils import pairwise_collision, get_distance, multiply, set_pose, \
...
def get_cfree_pose_pose_test(collisions=True, **kwargs):
    def test(b1, p1, b2, p2):
        """
        test if b1 at p1 will collide with b2 at p2

        Args:
            b1 (_type_): _description_
            p1 (_type_): intended Pose() of b1
            b2 (_type_): _description_
            p2 (_type_): Pose of b2
...
def get_cfree_approach_pose_test(problem, collisions=True):
    ...
    def test(b1, p1, g1, b2, p2):
        """
        test if the grasp for b1 collide with b2

        Args:
            b1 (_type_): movable object to grasp
            p1 (_type_): position of b1
            g1 (_type_): grasp for b1
            b2 (_type_): other objects
            p2 (_type_): position of other objects
...
def get_cfree_traj_pose_test(robot, collisions=True):
    def test(c, b2, p2):
        """
        test if trajectory c will collide with b2 at p2

        Args:
            c (_type_): a trajectory
            b2 (_type_): other movable obstacles
            p2 (_type_): position of b2
