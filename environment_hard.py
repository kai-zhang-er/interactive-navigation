...
from env import convert_imagepos_to_pos, convert_pos_to_imagepos, create_env2, generate_no_collision_pose, is_plan_possible, plan_pr2_base_motion, reset_movable_obstacles, get_ray_info_around_point, reset_movable_obstacles2, \
...
class NAMOENV(gym.Env):
    ...
    def __init__(self, init_pos=(-4,-1,0),
        ...
        self.reward_map=np.load("cost_map.npy")/100
        ...
        self.action_space=spaces.Box(low=np.array([-0.95,-np.pi/4.0]), high=np.array([0.95,np.pi/4.0]), shape=(2,), dtype=np.float32)
        ...
        low=np.array(self._N_RAYS*[_SCAN_RANGE_MIN]+[0.0]+[-math.pi]+[-12, -10])
        high=np.array(self._N_RAYS*[_SCAN_RANGE_MAX]+[math.inf]+[math.pi]+[12, 10])
    ...
    def _update_observation(self):
        ...
        self.observation=np.concatenate((obs_ray, distance, angle, self.robot_pos[:2]), axis=None)
    ...
    def reset(self):
        ...
        self.robot_pos[:2]=generate_no_collision_pose([[-8,-5],[-5,5]])
        # self.goal_pos[:2]=generate_no_collision_pose(self.base_limits)
        self.goal_pos[:2]=np.array([8.3,5.1])
    ...
    def step(self, action):
        ...
        img_pos=convert_pos_to_imagepos(self.robot_pos, [self.reward_map.shape[1], self.reward_map.shape[0]])
        reward-=self.reward_map[img_pos[1],img_pos[0]]
