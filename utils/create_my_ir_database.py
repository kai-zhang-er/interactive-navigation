...
from pybullet_tools.pr2_utils import close_arm, create_gripper, open_arm, set_arm_conf, get_other_arm, arm_conf, REST_LEFT_ARM, \
...
from pybullet_tools.utils import Pose, create_box, disconnect, add_data_path, connect, get_movable_joints, get_joint_positions, \
    sample_placement, set_pose, multiply, invert, set_joint_positions, pairwise_collision, inverse_kinematics, \
    get_link_pose, get_body_name, wait_for_duration, write_pickle, uniform_pose_generator, set_base_values, \
    load_pybullet, HideOutput, wait_if_gui, draw_point, point_from_pose, has_gui, elapsed_time, \
    sub_inverse_kinematics, BodySaver
from pybullet_tools.pr2_problems import create_pr2, create_table
...
def create_inverse_reachability(robot, body, arm, grasp_type, max_attempts=200, num_samples=100):
    ...
    while len(gripper_from_base_list) < num_samples:
        ...
        for attempt in range(max_attempts):
            ...
            # gripper_from_base_list.append(get_base_pose(robot))
...
def main():
    ...
    parser.add_argument('-grasp', default="top")
    parser.add_argument('-viewer', default=False, help='enable viewer.')
    ...
    create_inverse_reachability(pr2, box, arm=arm, grasp_type=grasp_type)
    # create_inverse_reachability2(pr2, box, table, arm=arm, grasp_type=grasp_type)
    disconnect()
