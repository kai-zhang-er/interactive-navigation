...
def NewPose(point=None, euler=None):
    point = Point() if point is None else point
    euler = Euler() if euler is None else euler
    return point, quat_from_euler(euler)
...
def sample_placement_on_aabb(top_body, bottom_aabb, top_pose=unit_pose(),
                             percent=1.0, max_attempts=50, epsilon=1e-3, set_obj=True):
...
def sample_reachable_base_with_preference(robot_pos, point, reachable_range=(0.3, 0.8)):
    """
    sample reachable base position and orientation with preference on robot position

    Args:
        robot_pos (_type_): _description_
        point (_type_): _description_
        reachable_range (tuple, optional): _description_. Defaults to (0.25, 1.0).
    """
    radius = np.random.uniform(*reachable_range)
    yaw = np.random.uniform(*CIRCULAR_LIMITS)
    to_robot_angle=get_angle(point[:2],robot_pos[:2])
    # sampled_angle= sample_norm(mu=to_robot_angle, sigma=PI/4, lower=to_robot_angle-PI/2, upper=to_robot_angle+PI/2)
    # x, y = radius*unit_from_theta(sampled_angle) + point[:2]
    x, y = radius*unit_from_theta(np.random.uniform(to_robot_angle-np.pi/4, to_robot_angle+np.pi/4)) + point[:2]
    base_values = (x, y, yaw)
    #set_base_values(robot, base_values)
    return base_values

def normal_pose_generator(robot_pos, gripper_pose, **kwargs):
    point = point_from_pose(gripper_pose)
    while True:
        base_values = sample_reachable_base_with_preference(robot_pos, point, **kwargs)
