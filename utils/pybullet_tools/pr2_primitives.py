...
from .pr2_utils import TOP_HOLDING_LEFT_ARM, SIDE_HOLDING_LEFT_ARM, GET_GRASPS, get_base_pose, get_gripper_joints, \
...
from .utils import invert, multiply, get_name, normal_pose_generator, set_pose, get_link_pose, is_placement, \
...
def get_stable_gen(problem, collisions=True,set_obj=True, **kwargs):
    ...
    def gen(body, surface):
        ...
        while True:
            ...
            body_pose = sample_placement(body, surface, set_obj=set_obj, **kwargs)
...
def get_ir_sampler(problem, custom_limits={}, max_attempts=25, collisions=True, learned="normal"):
    robot = problem.robot
    obstacles = problem.fixed if collisions else []
    ...
    def gen_fn(arm, obj, pose, grasp):
        """
        sample and find the reachable position and orientation to grasp object

        Args:
            arm (str): the arm to use, either left or right
            obj (int): object id to grasp
            pose (Pose): pose of obj
            grasp (Grasp): grasp
        ...
        elif learned=="uniform":
            base_generator = uniform_pose_generator(robot, gripper_pose) # (x,y,yaw)
        elif learned=="normal":
            robot_pos=base_values_from_pose(get_base_pose(robot))
            base_generator=normal_pose_generator(robot_pos, gripper_pose)
...
def get_ik_ir_gen(problem, max_attempts=25, learned="uniform", teleport=False, **kwargs):
