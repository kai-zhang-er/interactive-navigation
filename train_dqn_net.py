...
from environment_explore_graph import NAMOENV
...
init_obs=env.reset()
obs=process_observation(init_obs)
...
def optimize_model():
    ...
    non_final_next_states = torch.concat([s for s in batch.next_state if s is not None])
    state_batch = torch.concat(batch.state)
    action_batch = torch.concat(batch.action)
...
for i_episode in range(num_episodes):
    ...
    for t in range(500):
        ...
        state=process_observation(raw_obs)
        if not done:
            next_state = state
        else:
            next_state = None
    ...
    print("episode {}: average reward {}, steps {}".format(i_episode, sum(step_reward_list)/len(step_reward_list), n))
